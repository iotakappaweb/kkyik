/* Originally written for the Psi chapter at UCLA.
   ...
   Adapted once again for the Iota Kappa chapter at Boise State University by Bree Latimer (Psi).
...
function traverse(arr, tree, parent) {
   ...
   // filter out child elements to only obtain ordered lists (<ol>)
...
// obtains the family tree from the Iota Kappa Wiki
function getTree() {
   var data;
   ...
   $.ajax({
      ...
      success: function(result) {
         ...
         var fam = $("ol", $(html)).filter(function() {
            if (this.parentElement.id == "mw-content-text")
               return this;
         });

         // add root node to the array
         data = [{"name": "Iota Kappa Chapter", "class": "",  "active": "n", "parent": "null" }];

         // for each parent node (the Alphas of each family), traverse their branches and add to array
         for (var i = 0; i < fam.length; i++) {
            data.push({"name": "Family " + (i+1), "class": "", "active": "n", "parent": "Iota Kappa Chapter" });
